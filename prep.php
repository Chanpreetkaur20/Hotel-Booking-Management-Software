...
<link rel="stylesheet" href="css/datepicker.css">
	...
	$(document).ready(function(){
                    ...
                    <div class="panel-heading">
                    	<div class="row">
                        	<div class="col-lg-10 col-md-10 col-sm-8 col-xs-6">
                            	<h3 class="panel-title">Pre Purchase</h3>
                            </div>
                            <div class="col-lg-2 col-md-2 col-sm-4 col-xs-6" align="right">
                            	<button type="button" name="add" id="add_button" data-toggle="modal" data-target="#prepModal" class="btn btn-success btn-xs">Add</button>
                        	</div>
                        </div>
                   	...
                   	<div class="panel-body">
                   		<div class="row"><div class="col-sm-12 table-responsive">
                   			<table id="prep_data" class="table table-bordered table-striped">
                   				<thead>
									<tr>
                                    <th>ID</th>
		...
		<div id="bookingModal" class="modal fade">
        	<div class="modal-dialog">
        		<form method="post" id="booking_form" autocomplete="off">
					...
					<input type="hidden" name="prep_id" id="prep_id1" />
        ...
        <div id="prepModal" class="modal fade">
        	<div class="modal-dialog">
        		<form method="post" id="prep_form" autocomplete="off">
        			<div class="modal-content">
        			<div class="modal-header">
        				<button type="button" class="close" data-dismiss="modal">&times;</button>
						<h4 class="modal-title"><i class="fa fa-plus"></i> Add Pre Purchase</h4>
        			</div>
        			<div class="modal-body">
                    <div class="form-group">
                            <label>Enter Hotel Name</label>
...
$(document).ready(function(){
	...
	$('#add_button').click(function(){
		$('#prep_form')[0].reset();
		$('.modal-title').html("<i class='fa fa-plus'></i> Add Pre Purchase");
		$('#action').val("Add");
		$('#btn_action').val("Add");
	});

	var prepdataTable = $('#prep_data').DataTable({
		"processing": true,
		"serverSide": true,
		"order": [],
		"ajax":{
	...
	$(document).on('submit', '#prep_form', function(event){
	...
	$(document).on('click', '.book', function(){
		...
		$.ajax({
			url:"prep_action.php",
			method:"POST",
			data:{prep_id:prep_id, btn_action:btn_action},
			dataType:"json",
			success:function(data)
			{
	...
	$(document).on('click', '.update', function(){
		...
		$.ajax({
			...
			success:function(data)
			{
				...
				$('#gst').val(data.gst);
				$('.modal-title').html("<i class='fa fa-pencil-square-o'></i> Edit Pre Purchase");
				$('#prep_id').val(prep_id);
	...
	$(document).on('click', '.delete', function(){
		...
		if(confirm("Are you sure you want to change status?"))
		{
			$.ajax({
				url:"prep_action.php",
				method:"POST",
				data:{prep_id:prep_id, status:status, btn_action:btn_action},
				success:function(data)
				{
